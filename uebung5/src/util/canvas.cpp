...
void Canvas::animationFrameChanged(const float animationFrame)
{
    if(m_painter && (animationFrame != m_painter->getAnimationFrame()))
    {
        m_painter->setAnimationFrame(animationFrame);
        m_update = true;
    }
}
