...
AbstractPainter::AbstractPainter(const float animationFrame)
    : m_camera(0)
...
void AbstractPainter::setAnimationFrame(const float animFrame)
{
...
bool AbstractPainter::onMouseReleased(QMouseEvent *event)
{
    return false;
}
...
QOpenGLShaderProgram* AbstractPainter::createBasicShaderProgram(const QString &vertexShaderFileName, const QString &geometryShaderFileName, const QString &fragmentShaderFileName)
{
    QOpenGLShaderProgram * program = new QOpenGLShaderProgram();
    program->create();
