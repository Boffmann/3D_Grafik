...
QString Canvas::querys(OpenGLFunctions & gl, GLenum penum)
{
...
GLint Canvas::queryi(OpenGLFunctions & gl, GLenum penum)
{
    GLint result;
...
void Canvas::initializeGL(const QSurfaceFormat & format)
{
    ...
    auto functions = dynamic_cast<OpenGLFunctions*>(m_context->versionFunctions<OpenGLFunctions>());
    ...
    if (functions != nullptr)
    {
        ...
        qDebug();
        qDebug().nospace() << "GPU: "
            << qPrintable(querys(*functions, GL_RENDERER)) << " ("
            << qPrintable(querys(*functions, GL_VENDOR)) << ", "
            << qPrintable(querys(*functions, GL_VERSION)) << ")";
        qDebug().nospace() << "GL Version: "
            << qPrintable(QString::number(queryi(*functions, GL_MAJOR_VERSION))) << "."
            << qPrintable(QString::number(queryi(*functions, GL_MINOR_VERSION))) << " "
            << (queryi(*functions, GL_CONTEXT_CORE_PROFILE_BIT) ? "Core" : "Compatibility");
        qDebug();
...
{
    if (m_painter) {
        if (m_painter->onKeyPressed(event)) {
            m_update = true;
        }
    }

    if (m_navigation && !event->isAccepted())
    {
...
void Canvas::mouseReleaseEvent(QMouseEvent * event)
{
    if (m_painter) {
        m_context->makeCurrent(this);
        if (m_painter->onMouseReleased(event)) {
            m_update = true;
        }
        m_context->doneCurrent();
    }
